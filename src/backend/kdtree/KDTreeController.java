...
public class KDTreeController {
	...
	public KDTreeController() throws IOException, MapsException {
		try {
			...
			if (mapNodesList.isEmpty()) {
				throw new MapsException("<KDTree Controller> Empty dataset. Cannot construct KDTree");
			}
			tree = new KDTree<>(2, mapNodesList, mapNodesList.get(0).getComparators());
		} catch (final KDTreeException e) {
			throw new MapsException(e.getMessage());
		}
	}
